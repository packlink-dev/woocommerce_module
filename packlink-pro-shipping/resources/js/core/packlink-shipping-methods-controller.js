...
(function () {
    function ShippingMethodsController(configuration) {
        ...
        function addFixedPriceCriteria(event) {
            ...
            if (currentCriteria.to && typeof currentCriteria.to === 'number' && currentCriteria.to > currentCriteria.from) {
        ...
        function onFixedPriceToBlur(event) {
            ...
            if (value !== '' && !isNaN(value)) {
                if (index < methodModel.fixedPricePolicy.length - 1) {
                    ...
                    if (
                        successor.from &&
                        (typeof successor.to === 'number' && successor.to > numericValue || typeof successor.to !== 'number')
                    ) {
                        successor.from = numericValue;
                        if (
                            (index + 1 === methodModel.fixedPricePolicy.length - 1) &&
                            typeof successor.to === 'number' && successor.to <= successor.from
                        ) {
                            successor.to = '';
                        }
                    }
